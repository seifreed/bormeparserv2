...
class BormeATestCase(unittest.TestCase):
    ...
    def test_instance(self):
        ...
        self.assertEqual(
            self.borme.filename, os.path.join(EXAMPLES_PATH, "BORME-A-2015-27-10.pdf")
        )
...
class FakeBormeTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        bormeanuncios = [
            BormeAnuncio(
                1, DATA1[214]["Empresa"], DATA1[214]["Actos"], DATA1[214]["Extra"]
            )
        ]
        cls.borme = Borme(
            (2015, 2, 10),
...
class BormeAnuncioTestCase(unittest.TestCase):
    ...
    def test_instance(self):
        ...
        self.assertEqual(
            self.anuncio.datos_registrales, DATA1[214]["Actos"][1]["Datos registrales"]
        )
    ...
    def test_get_actos(self):
        ...
        self.assertEqual(
            actos[0], ("Ceses/Dimisiones", DATA1[214]["Actos"][0]["Ceses/Dimisiones"])
        )
        self.assertEqual(
            actos[1], ("Constitución", DATA1[214]["Actos"][2]["Constitución"])
        )
        self.assertEqual(
            actos[2], ("Nombramientos", DATA1[214]["Actos"][3]["Nombramientos"])
        )
...
class BormeActoTestCase(unittest.TestCase):
    def test_cargo_instance(self):
        ...
        self.assertEqual(
            acto.cargos["Adm. Unico"],
            DATA1[214]["Actos"][3]["Nombramientos"]["Adm. Unico"],
        )
...
class BormeXMLTestCase(unittest.TestCase):
    ...
    def test_get_url_pdfs(self):
        ...
        url_cve_a = {
            "BORME-A-2015-183-06": "https://www.boe.es/borme/dias/2015/09/24/pdfs/BORME-A-2015-183-06.pdf"
        }
        ...
        url_cve_b = {
            "BORME-B-2015-183-04": "https://www.boe.es/borme/dias/2015/09/24/pdfs/BORME-B-2015-183-04.pdf"
        }

        urls_c = {
            "BORME-C-2015-%s.xml"
            % str(x): "https://www.boe.es/diario_borme/xml.php?id=BORME-C-2015-%s"
            % str(x)
        ...
        self.assertEqual(
            self.bxml.get_url_pdfs(seccion=SECCION.A, provincia="BADAJOZ"), url_cve_a
        )
    ...
    def test_get_cves(self):
        ...
        seccion_b_bormes = [
            "BORME-B-2015-183-%s" % x for x in ["04", "07", "15", "23", "28", "29"]
        ]

        seccion_c_bormes = ["BORME-C-2015-%s" % str(x) for x in range(9348, 9374 + 1)]
        ...
        self.assertEqual(
            self.bxml.get_cves(), seccion_a_bormes + seccion_b_bormes + seccion_c_bormes
        )
    ...
    def test_get_sizes(self):
        ...
        self.assertEqual(
            self.bxml.get_sizes(SECCION.A, "MADRID"), {"BORME-A-2015-183-28": 517865}
        )
        self.assertEqual(self.bxml.get_sizes(SECCION.B), seccion_b_sizes)
        self.assertEqual(
            self.bxml.get_sizes(SECCION.B, "MADRID"), {"BORME-B-2015-183-28": 153207}
        )
...
class BormeCTestCase1(unittest.TestCase):
    ...
    def test_instance(self):
        self.assertEqual(
            self.borme["cifs"], {"A31017494", "A31067218", "A58348038", "B31136005"}
        )
        self.assertEqual(self.borme["cve"], "BORME-C-2011-20488")
        self.assertEqual(self.borme["departamento"], "CONVOCATORIAS DE JUNTAS")
        self.assertEqual(
            self.borme["empresa"], "DESARROLLOS ESPECIALES DE SISTEMAS DE ANCLAJE, S.A."
        )
